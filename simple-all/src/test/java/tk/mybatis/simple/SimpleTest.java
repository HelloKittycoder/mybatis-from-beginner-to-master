...
public class SimpleTest {
    ...
    public void test() throws SQLException {
        ...
        dataSource.setUrl("jdbc:mysql://localhost:3306/mybatis?serverTimezone=UTC&useUnicode=true&characterEncoding=UTF-8&useSSL=false");
        ...
        // 创建静态 SqlSource
        // 最简单的，相当于从 xml 获取接口注解获取 SQL，创建合适的 SqlSource 对象
        ...
        // ResultMap resultMap = new ResultMap.Builder(config, "defaultResultMap", Country.class, new ArrayList<>()).build();
        ...
        // 创建 MappedStatement
        MappedStatement.Builder msBuiler = new MappedStatement.Builder(
                config, "tk.mybatis.simple.SimpleMapper.selectCountry", sqlSource, SqlCommandType.SELECT);
        msBuiler.parameterMap(paramBuilder.build());
        List<ResultMap> resultMaps = new ArrayList<>();
        resultMaps.add(resultMap);
        // 设置返回值的 resultMap
        msBuiler.resultMaps(resultMaps);
        // 设置缓存
        msBuiler.cache(countryCache);
        // 创建 ms
        MappedStatement ms = msBuiler.build();
